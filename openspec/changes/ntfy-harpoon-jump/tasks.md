...
- [ ] 1.1 Add a pure pane-id parser: `terminal_N` | bare `N` → `Option<u32>`
...
- [ ] 1.2 Add a pure `slot_for_pane` lookup over `BookmarkStore`
...
- [ ] 2.1 Implement `ZellijPlugin::pipe()` on `State`: match message name
  ...
  - allow_new_files: false
- [ ] 2.2 In `pipe()`, match message name `jump_pane` → parse pane id, call the
...
- [ ] 3.1 Unit tests for the pane-id parser: `terminal_7`→7, `7`→7,
  ...
  - allow_new_files: true
- [ ] 3.2 Unit tests for `slot_for_pane`: harpooned id → Some(1-based), absent id
