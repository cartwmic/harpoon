...
- [x] 0.1 **Render coordinate verification** — VERIFIED in zellij 0.44.1 floating plugin pane: `y=0` IS visible (contradicts the stale `// Note: y=0 overlaps with the zellij pane frame/title bar` comment in pre-existing `main.rs`). Both `Y0_PROBE` and `Y1_PROBE` markers rendered. `HEADER_BASE_Y = 0`. The pre-existing render code's `let mut y = 1` was a false-pessimistic workaround. **Tiled-pane behavior may differ; verify before final release**
- [x] 0.2 **Plugin instance lifecycle verification** — VERIFIED. `load_addr` fired ONCE at instantiation (`0x13f800`); ALL subsequent `update_addr` values matched `0x13f800` across many events including hide-via-toggle-floating-panes followed by re-show. Plugin `State` survives `hide_self()` — close-helper mode/query reset design is sound
- [x] 0.3 **`BareKey::Char('K')`/`('J')` shift-detection probe** — VERIFIED. Host emits **BOTH simultaneously**: `Char('K') + modifiers={Shift}` and `Char('J') + modifiers={Shift}`. The case is uppercase AND the Shift modifier is present. **FFI normalization rule confirmed**: when input is `Char(c) + Shift` where `c` is ASCII alphabetic, drop the Shift modifier (case already encodes it) so handlers see `Char('K') + ModifierSet::PLAIN`. Plain unmodified k/j arrives as `Char('k')/Char('j') + modifiers={}`
- [x] 0.4 **`nucleo-matcher` wasm + index-semantics spike** — PASS: `nucleo-matcher = "0.3.1"` (default features) compiles for wasm32-wasip1, returns char indices (verified: `📦 build` + `b` → `[2]`). Use this dep, NOT in-tree. Note: `default-features = false` breaks; keep defaults: add `nucleo-matcher` to a throwaway branch with `--no-default-features` plus the minimum needed for `Matcher::fuzzy_indices`; run `cargo build --target wasm32-wasip1`. In the same branch, write a unit test that feeds haystack `"📦 build"` and needle `"b"` through the matcher's `Utf32Str`/`Utf32String` constructor and asserts the returned index is `2` (char position) NOT `4` (byte offset). Decide fuzzy backend based on results; if char-index semantics or wasm build fails, switch to in-tree matcher
- [x] 0.5 **Render-side multi-byte highlight verification** — VERIFIED via direct source inspection of zellij-server-0.44.1 (more reliable than visual capture; zellij 0.44.1's `dump-screen --ansi` returns 0 bytes — a regression). At `zellij-server-0.44.1/src/ui/components/text.rs:55`, the host iterates: `for (i, character) in text.text.chars().enumerate()` and applies styling when `i` is in the indices vec. The host treats `Text::color_indices` and `Text::color_range` indices as **character (Unicode scalar) positions, NOT byte offsets**. **No char→byte conversion needed at the FFI render shim**. The matcher returns char indices, the render layer passes them straight through. Phase 0.5 fallback path (task 7.8) is NOT needed and can be marked obsolete

## 1. Workspace restructure & foundation

- [x] 1.1 Convert root `Cargo.toml` to a workspace with two members: `harpoon-core` (new lib, no `zellij-tile` dep, native+wasm) and `harpoon-plugin` (existing crate moved into `harpoon-plugin/`, cdylib, depends on `harpoon-core` and `zellij-tile`). The cdylib package name remains `harpoon` so the built artifact path stays `target/wasm32-wasip1/release/harpoon.wasm`
- [x] 1.1b **Move target pin**: relocate `.cargo/config.toml` (with its `wasm32-wasip1` target pin) from the repo root into `harpoon-plugin/.cargo/config.toml`. Workspace root has NO target pin so `cargo test -p harpoon-core` from root runs natively. **Important**: cargo walks `.cargo/config.toml` from the cwd upward, NOT from the package directory — so `cargo build -p harpoon` from workspace root will NOT pick up the wasm pin. Plugin builds must either `cd harpoon-plugin/ && cargo build --release` OR `cargo build -p harpoon --target wasm32-wasip1 --release` from root with explicit `--target`
- [x] 1.2 Define **host-agnostic projection types** in `harpoon-core/src/lib.rs`:
  ```rust
...
- [x] 1.3 In `harpoon-core`, create `mode.rs` with `enum Mode { Command, Filter, Jump }` and parse helpers (case-insensitive)
- [x] 1.4 In `harpoon-core`, create `config.rs` with `Config { default_mode, matcher, show_slots }` and `parse_from_btree(map: &BTreeMap<String,String>) -> Config` honoring fallback rules; use `eq_ignore_ascii_case` for `default_mode`, `matcher`, AND `show_slots`
- [x] 1.5 In `harpoon-core`, create `effect.rs` with `enum Effect { Render, Close, FocusPane(u32), Save, Noop }`. Document that effects in a `Vec<Effect>` are applied in declared order
- [x] 1.6 Verify `cargo test -p harpoon-core` from workspace root compiles and runs an empty test using the host's default native target. This validates the workspace + target-pin split before any tests depend on it — 33 tests pass
- [x] 1.7 Add fields to the plugin's `State`: `mode: Mode`, `default_mode: Mode`, `query: String`, `config: Config`, `matcher: MatcherImpl` (NOT `Box<dyn Matcher>` — see decision in design.md). `MatcherImpl` derives `Default` so `State` keeps `#[derive(Default)]`
- [x] 1.8 In `ZellijPlugin::load`: after `Config::parse_from_btree` populates `state.config`, set `state.matcher = MatcherImpl::from_config(&state.config)`. Add a unit test (in `harpoon-core` if `from_config` is fully testable there, else `harpoon-plugin/tests/`) asserting that `from_config(&Config { matcher: Substring, .. })` returns the `MatcherImpl::Substring(_)` variant
...
- [x] 3.4 Plugin shim FFI conversion. Implement:
...
- [x] 3.6 Plugin shim: `close_helper(&mut self)` calls `hide_self()`, sets `self.mode = self.default_mode`, clears `self.query`, then calls `reanchor_selected_to_focus(&mut self.dispatch_state, focused_idx(&self.dispatch_state.panes, self.dispatch_state.focused_pane_id))` to re-anchor `selected` immediately (no longer relying on a subsequent `PaneUpdate`). View-length clamp via `clamp_selected_to_view` is the next render path's responsibility — close itself only re-anchors
- [x] 3.7 Replace every direct `hide_self()` callsite with `Effect::Close` returns from handlers (effects flow through `close_helper` in 3.5)
- [x] 3.8 In `harpoon-core`: `impl DispatchState { pub fn selected_pane_index(&self, filtered_indices: &[usize]) -> Option<usize> { ... } }`. Returns the underlying `panes` index for the current `selected`: in command/jump mode (or filter+empty-query), uses `selected` directly with bounds check; in filter mode with non-empty query, indexes `filtered_indices[selected]`. Plugin shim is a thin pass-through
...
- [x] 3.10 (alt landed) FFI conversion landed inline as `key_event_to_input` in `harpoon-plugin/src/main.rs` rather than as a separate `parse_input_key` primitive in `harpoon-core/src/key.rs`. Phase 0.3 verified the host's emission style live (Char(uppercase) + Shift); the inline conversion encodes the normalization rule directly. Standalone primitive shim deferred since the conversion is short (~25 lines) and tested via the integration smoke (live zellij verified) (lives in `harpoon-core`, native-testable). Tests live in `harpoon-core/src/key.rs`'s `#[cfg(test)] mod tests`: `parse_input_key("Char", Some('a'), false, false, false, false)` → `InputKey::Char('a', empty)`; `parse_input_key("Char", Some('a'), true, false, false, false)` → `InputKey::Char('a', {ctrl})`; `parse_input_key("Char", Some('k'), false, false, true, false)` → `InputKey::Char('K', empty)` (shift normalization); `parse_input_key("Char", Some('K'), false, false, false, false)` → `InputKey::Char('K', empty)` (already canonical); `parse_input_key("Backspace", None, ...)` → `InputKey::Backspace`. Plugin's `From<zellij_tile::Key>` is a thin one-line wrapper delegating to this primitive shim
- [x] 3.11 Run `cargo test -p harpoon-core dispatch::` and confirm all dispatch tests pass before moving on — 217 tests pass

## 4. Command mode

- [x] 4.0 Implement `pub fn freeze_on_user_mutation(state: &mut DispatchState, persistence: &mut Persistence)` in `harpoon-core` (or the plugin shim if `Persistence` lives there — helper signature stays the same). Algorithm per design.md "State.panes is a sparse Vec":
...
- [x] 4.1 Implement `handle_command_key(state, ctx, key)`. Each branch enumerates its effects explicitly. ASCII-letter and digit arms gate on `modifiers.is_plain()` (post-normalization); modified inputs return `vec![Effect::Noop]` UNLESS noted otherwise. Symbol-key arms (`/`, `#`) accept any modifier set:
...
- [x] 4.2 `/` (any modifier set, since `#`/`/` may arrive with implicit Shift on some keyboard layouts) → set mode to `Filter`, return `vec![Effect::Render]`
- [x] 4.3 `#` (any modifier set, same reasoning as `/`) → set mode to `Jump`, return `vec![Effect::Render]`
- [x] 4.4 `Esc` → return `vec![Effect::Close]`
- [x] 4.5 **Digit-only** slot keys (`1-9`, modifiers plain): resolve via `state.panes.get(slot_index).and_then(Option::as_ref)`. If `Some(pane)` (Live), return `[Effect::Close, Effect::FocusPane(pane.id)]`. If `Some(None)` (placeholder — unresolved bookmark with that saved index), return `vec![]` (no-op; preserves saved-position contract during restore window). If `None` (slot beyond list length), return `vec![]`. Letter keys (`a-z`) MUST follow their existing command bindings or remain unbound; do NOT add letter slot jumps in command mode
- [x] 4.6 `K` (Char('K') OR Char('k')+shift, modifier-set normalized) reorder up: pre-conditions — `panes.len() >= 2 && selected > 0 && selected < panes.len() && panes[selected].is_some() && panes[selected-1].is_some()` (both targets must be Live, no swapping with placeholders). If pre-conditions fail, return `vec![]` (no freeze, no Save, no Render). Else: **call `freeze_on_user_mutation(state, persistence)` first** (post-freeze, panes is dense and selected has re-anchored to the same pane id), THEN swap `panes[selected]` with `panes[selected-1]`, swap the corresponding bookmarks' `index` fields (both bookmarks are reachable via `pane_id_to_bookmark_idx`), decrement `selected`, return `[Effect::Save, Effect::Render]`. The Phase 0 probe (task 0.3) determines which form the host emits; the handler accepts BOTH forms defensively, since `harpoon-core::InputKey` normalization can collapse them
- [x] 4.7 `J` reorder down: same modifier handling as K. Pre-conditions — `panes.len() >= 2 && selected < panes.len() - 1 && panes[selected].is_some() && panes[selected+1].is_some()`. If pre-conditions fail, return `vec![]`. Else: **freeze first**, swap, swap matching bookmarks' `index`, increment `selected`, return `[Effect::Save, Effect::Render]`
- [x] 4.8 **Order-preservation refactor (P0 fix)**: implemented inside 4.1 (`a`/`A` branches). The legacy `sort_panes()` is NOT called from these paths. `A` iterates panes in `(tab.position ASC, PaneInfo.id ASC)` order so repeated runs produce the same result
- [x] 4.9 Write failing unit tests FIRST:
...
- [x] 7.2 In `harpoon-core`, `pub fn build_header(state: &DispatchState, cols: usize, max_height: u16) -> RenderHeader`: emits standard `==== N panes ====` for command/jump or `/<query> (<m>/<n>)` for filter with non-empty query; appends `[N]/[F]/[J]` badge; returns `Vec<HeaderLine>` + consumed height (≤ max_height). Implements narrow-width truncation order (drop count first; then if max_height ≥ 2, badge moves to its own line; then truncate query with leading ellipsis). At max_height == 1, badge stays inline and query is truncated more aggressively
- [x] 7.3 In `harpoon-core`, `pub fn build_rows(state: &DispatchState, placeholder_lookup: &dyn Fn(usize) -> Option<(String, String)>, matcher: &mut MatcherImpl, max_rows: u16) -> Vec<RenderRow>`: walks `state.panes` directly. For each `i in 0..panes.len()`:
...
- [x] 7.8 **OBSOLETE per Phase 0.5 verification**: zellij-server-0.44.1 iterates `text.chars().enumerate()` (verified by direct source inspection; see task 0.5). Host indexes by char, not byte. No `char_indices_to_bytes` helper needed. Task closed without action

## 8. Persistence schema upgrade & restore buffer

- [x] 8.1 In `harpoon-plugin/src/persistence.rs`, add `index: Option<u16>` field to `PaneBookmark` (with `#[serde(default)]` so missing field deserializes to `None`), define the v2 envelope wrapping a single bookmarks Vec, and add the pane-id↔bookmark identity map as a first-class field on `Persistence`:
  ```rust
...
- [x] 8.3 Load path: try to deserialize as `PersistedV2` first (v2 envelope). If that fails, fall back to `Vec<PaneBookmark>` (v1 bare array). On v1 success: assign `index = Some(i as u16)` in array order. (No explicit "mark for migration" needed — every save writes v2 envelope, so the next save naturally writes v2.) On both fail: log `LoadFromDiskFailed` and start with empty bookmarks
- [x] 8.4 **Sparse-Vec restore model**: `state.panes: Vec<Option<Pane>>` and `Persistence::bookmarks: Vec<PaneBookmark>` carrying `index: Option<u16>`. Resolution semantics on each `update_panes` round:
